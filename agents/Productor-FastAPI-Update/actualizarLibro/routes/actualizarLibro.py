...
from schemas.dto import LibroUpdateDTO
from config.rabbitConf import send_mensage, COMMON_ROUTING_KEY ,SOCORRO_ROUTING_KEY , MALAGA_ROUTING_KEY, BARBOSA_ROUTING_KEY

router = APIRouter()

@router.put("/actualizar-libro/{destino}")
async def eliminar_libro(destino: str, libro_update: LibroUpdateDTO):
    ...
    return await send_mensage(libro_update, key)
